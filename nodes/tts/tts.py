...
class tts_node_v2(BRANDNode):
    ...
    def __init__(self):
        ...
        self.tts = stts2(
                model_path='/samba/t15/T15_TTS_Model/version8-StyleTTS2',
                ref_audio_path='/samba/t15/T15_TTS_Model/version8-StyleTTS2/wavs/audio1.wav_0.wav',
                whisper_model_size = self.whisper_model_size,
                device = self.device,
                fs = self.fs,
            )
        
        logging.info('Starting audio output stream...')
        print(sd.query_devices())
        self.stream = sd.OutputStream(channels=1, samplerate=self.fs)
        self.stream.start()

        logging.info('TTS_node_V2 initialized and ready to go.')
    ...
    def play_audio_from_mem(self, wav, rate):
        ...
        self.stream.write(np.array(wav, dtype=np.float32))
...
class stts2():
    def __init__(self, model_path, ref_audio_path, fs=24000, whisper_model_size='base', device='cuda:0'):
        ...
        self.device = device
        self.fs = fs
        self.config = yaml.safe_load(open(os.path.join(self.model_path, "config_ft.yml")))
